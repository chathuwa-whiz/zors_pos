...
export default function ProductStats({ products }: ProductStatsProps) {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      color: lowStockItems > 0 ? 'bg-yellow-500' : 'bg-gray-500',
      bgColor: lowStockItems > 0 ? 'bg-yellow-50' : 'bg-gray-50',
      textColor: lowStockItems > 0 ? 'text-yellow-600' : 'text-gray-600'
    },
    {
      title: 'Out of Stock',
      value: outOfStockItems.toLocaleString(),
      icon: TrendingUp,
      color: outOfStockItems > 0 ? 'bg-red-500' : 'bg-gray-500',
      bgColor: outOfStockItems > 0 ? 'bg-red-50' : 'bg-gray-50',
      textColor: outOfStockItems > 0 ? 'text-red-600' : 'text-gray-600'
    }
  ];
